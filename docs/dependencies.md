# Dependencies

## System

- [Git](https://git-scm.com/)
- [Node.js](https://nodejs.org/)
- [pnpm](https://pnpm.io/)
- [Docker](https://www.docker.com/)

## Node.js

- [Svelte](https://svelte.dev/)
  - [SvelteKit](https://svelte.dev/docs/kit/introduction)
  - [Static Site Generation](https://svelte.dev/docs/kit/adapter-static)
  - [Svelte Check](https://svelte.dev/docs/cli/sv-check)
- [TypeScript](https://www.typescriptlang.org/)
- [Vite](https://vite.dev/)
  - [Svelte Plugin](https://github.com/sveltejs/vite-plugin-svelte)
- [PostCSS](https://postcss.org/)
  - [Autoprefixer](https://github.com/postcss/autoprefixer)
  - [cssnano](https://cssnano.github.io/cssnano/)
  - [Preset Env](https://preset-env.cssdb.org/)
- [sanitize.css](https://csstools.github.io/sanitize.css/)
- [ESLint](https://eslint.org/)
  - [TypeScript Config](https://typescript-eslint.io/)
  - [Svelte Config](https://sveltejs.github.io/eslint-plugin-svelte/)
  - [Prettier Config](https://github.com/prettier/eslint-config-prettier)
  - [Compatibility Utilities](https://github.com/eslint/rewrite/tree/main/packages/compat)
  - [Globals](https://github.com/sindresorhus/globals)
- [Prettier](https://prettier.io/)
  - [Svelte Plugin](https://github.com/sveltejs/prettier-plugin-svelte)
- [Husky](https://typicode.github.io/husky/)
- [lint-staged](https://github.com/lint-staged/lint-staged)
- [Commitlint](https://commitlint.js.org/)
  - [Conventional Commits Config](https://github.com/conventional-changelog/commitlint/tree/master/%40commitlint/config-conventional)
- [czg](https://cz-git.qbb.sh/cli/)

## GitHub Actions

- [Release Please Action](https://github.com/marketplace/actions/release-please-action)
- [Checkout Action](https://github.com/marketplace/actions/checkout)
- [Setup pnpm Action](https://github.com/marketplace/actions/setup-pnpm)
- [Setup Node.js Action](https://github.com/marketplace/actions/setup-node-js-environment)
- [Upload Artifact Action](https://github.com/marketplace/actions/upload-a-build-artifact)
- [Download Artifact Action](https://github.com/marketplace/actions/download-a-build-artifact)
- [Wrangler Action](https://github.com/marketplace/actions/deploy-to-cloudflare-workers-with-wrangler)

## Cloud Services

- [Cloudflare Pages](https://developers.cloudflare.com/pages/)
  - [Cloudflare Wrangler](https://developers.cloudflare.com/workers/wrangler/)
